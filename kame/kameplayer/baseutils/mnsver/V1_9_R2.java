...
public class V1_9_R2 extends Vx_x_Rx {
	...
	public void w(org.bukkit.World w) {
		World world = ((CraftWorld)w).getHandle();
		for(TileEntity e : world.tileEntityList) {
			if(e instanceof TileEntityCommand) {
				CommandBlockListenerAbstract ee = ((TileEntityCommand) e).getCommandBlock();
				if(ee.getCommand().contains("score_kpTrig")) {
					ee.setCommand(ee.getCommand().replaceAll("score_kpTrig", "score_trig"));
					world.notify(e.getPosition(), e.getBlock().getBlockData(), e.getBlock().getBlockData(), 3);
				}
			}
		}
	}
	...
	public void copyBlockCommandNBT(org.bukkit.block.Block b, byte power) {
		...
		//data.doPhysics(world, pos, block);
		data.doPhysics(world, pos.south(), block);
		data.doPhysics(world, pos.north(), block);
		data.doPhysics(world, pos.east(), block);
		data.doPhysics(world, pos.west(), block);
	}
