...
public class V1_7_R4 extends Vx_x_Rx {
	...
	public void sendTabComplete(Player player, String message) {
		((CraftPlayer)player).getHandle().playerConnection.sendPacket(new PacketPlayOutTabComplete(new String[]{message}));
	}
	...
	public void setBlockObject(org.bukkit.Location l1, org.bukkit.Location l2) {
		World world = ((CraftWorld)l1.getWorld()).getHandle();
		Block block = world.getType(l1.getBlockX(), l1.getBlockY(), l1.getBlockZ());
		world.setTypeAndData(l2.getBlockX(), l2.getBlockY(), l2.getBlockZ(), block, l1.getBlock().getData(), 0);
	}
	...
	public void copyBlockEntity(org.bukkit.World w, int x, int y, int z, int i, int j, int k) {
		...
		world.setTypeAndData(i, j, k, block, world.getData(x, y, z), 1);
	...
	public void copyBlockCommandNBT(org.bukkit.block.Block b, byte power) {
		...
		if(tile != null && tile instanceof TileEntityCommand) {
			CommandBlockListenerAbstract cb = ((TileEntityCommand) tile).getCommandBlock();
			NBTTagCompound nbt = new NBTTagCompound();
			cb.a(nbt);
